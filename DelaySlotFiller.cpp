//===-- DelaySlotFiller.cpp - MWV208 delay slot filler
//---------------------===//
//
...
static cl::opt<bool>
    DisableDelaySlotFiller("disable-mwv208-delay-filler", cl::init(false),
                           cl::desc("Disable the Mwv208 delay slot filler."),
                           cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "MWV208 Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);
  bool runOnMachineFunction(MachineFunction &F) override {
    bool Changed = false;
    Subtarget = &F.getSubtarget<Mwv208Subtarget>();
  ...
  bool delayHasHazard(MachineBasicBlock::iterator candidate, bool &sawLoad,
                      bool &sawStore, SmallSet<unsigned, 32> &RegDefs,
                      SmallSet<unsigned, 32> &RegUses);

  MachineBasicBlock::iterator findDelayInstr(MachineBasicBlock &MBB,
...
/// createMwv208DelaySlotFillerPass - Returns a pass that fills in delay
/// slots in Mwv208 MachineFunctions
///
FunctionPass *llvm::createMwv208DelaySlotFillerPass() { return new Filler; }
...
bool Filler::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  bool Changed = false;
  Subtarget = &MBB.getParent()->getSubtarget<Mwv208Subtarget>();
  ...
  for (MachineBasicBlock::iterator I = MBB.begin(); I != MBB.end();) {
    ...
    if (!DisableDelaySlotFiller && (MI->getOpcode() == JJ::RESTORErr ||
                                    MI->getOpcode() == JJ::RESTOREri)) {
    ...
    if (!Subtarget->isV9() &&
        (MI->getOpcode() == JJ::FCMPS || MI->getOpcode() == JJ::FCMPD ||
         MI->getOpcode() == JJ::FCMPQ)) {
      BuildMI(MBB, I, MI->getDebugLoc(), TII->get(JJ::NOP));
      Changed = true;
      continue;
    }
    ...
    if (needsUnimp(MI, structSize)) {
      ...
      BuildMI(MBB, ++J, MI->getDebugLoc(), TII->get(JJ::UNIMP))
          .addImm(structSize);
...
Filler::findDelayInstr(MachineBasicBlock &MBB,
  ...
  if (Opc == JJ::RET || Opc == JJ::TLS_CALL)
    return MBB.end();

  if (Opc == JJ::RETL || Opc == JJ::TAIL_CALL || Opc == JJ::TAIL_CALLri) {
    MachineBasicBlock::iterator J = slot;
    --J;

    if (J->getOpcode() == JJ::RESTORErr || J->getOpcode() == JJ::RESTOREri) {
      // change retl to ret.
      if (Opc == JJ::RETL)
        slot->setDesc(Subtarget->getInstrInfo()->get(JJ::RET));
      return J;
    }
  }
...
bool Filler::delayHasHazard(MachineBasicBlock::iterator candidate,
  ...
  if (Subtarget->insertNOPLoad() && Opcode >= JJ::LDDArr && Opcode <= JJ::LDrr)
    return true;

  // Same as above for FDIV and FSQRT on some LEON processors.
  if (Subtarget->fixAllFDIVSQRT() && Opcode >= JJ::FDIVD &&
      Opcode <= JJ::FSQRTD)
    return true;
...
void Filler::insertDefsUses(MachineBasicBlock::iterator MI,
  ...
  for (const MachineOperand &MO : MI->operands()) {
    ...
    if (MO.isUse()) {
      ...
      if (MO.isImplicit() && MI->getOpcode() == JJ::RETL)
        continue;
      RegUses.insert(Reg);
    }
  }
...
static bool combineRestoreADD(MachineBasicBlock::iterator RestoreMI,
  ...
  AddMI->setDesc(TII->get((AddMI->getOpcode() == JJ::ADDrr) ? JJ::RESTORErr
                                                            : JJ::RESTOREri));

  // Map the destination register.
  AddMI->getOperand(0).setReg(reg - JJ::I0 + JJ::O0);
...
static bool combineRestoreOR(MachineBasicBlock::iterator RestoreMI,
  ...
  if (OrMI->getOpcode() == JJ::ORrr && OrMI->getOperand(1).getReg() != JJ::G0 &&
      OrMI->getOperand(2).getReg() != JJ::G0)
    return false;

  if (OrMI->getOpcode() == JJ::ORri && OrMI->getOperand(1).getReg() != JJ::G0 &&
  ...
  OrMI->setDesc(TII->get((OrMI->getOpcode() == JJ::ORrr) ? JJ::RESTORErr
                                                         : JJ::RESTOREri));

  // Map the destination register.
  OrMI->getOperand(0).setReg(reg - JJ::I0 + JJ::O0);
...
static bool combineRestoreSETHIi(MachineBasicBlock::iterator RestoreMI,
  ...
  assert(RestoreMI->getOpcode() == JJ::RESTORErr);

  RestoreMI->setDesc(TII->get(JJ::RESTOREri));

  RestoreMI->getOperand(0).setReg(reg - JJ::I0 + JJ::O0);
  RestoreMI->getOperand(1).setReg(JJ::G0);
...
bool Filler::tryCombineRestoreWithPrevInst(MachineBasicBlock &MBB,
  ...
  assert(MBBI->getOpcode() == JJ::RESTORErr &&
         MBBI->getOperand(0).getReg() == JJ::G0 &&
         MBBI->getOperand(1).getReg() == JJ::G0 &&
         MBBI->getOperand(2).getReg() == JJ::G0);
  ...
  case JJ::ADDrr:
  case JJ::ADDri:
    return combineRestoreADD(MBBI, PrevInst, TII);
    break;
  case JJ::ORrr:
  case JJ::ORri:
    return combineRestoreOR(MBBI, PrevInst, TII);
    break;
  case JJ::SETHIi:
    return combineRestoreSETHIi(MBBI, PrevInst, TII);
    break;
  }
