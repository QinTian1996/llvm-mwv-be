...
void ErrataWorkaround::insertNop(MachineBasicBlock::iterator I) {
  BuildMI(*I->getParent(), I, I->getDebugLoc(), TII->get(JJ::NOP));
}
...
bool ErrataWorkaround::isFloat(MachineBasicBlock::iterator I) {
  ...
  if (!JJ::FPRegsRegClass.contains(reg) && !JJ::DFPRegsRegClass.contains(reg))
    return false;
...
bool ErrataWorkaround::checkSeqTN0012(MachineBasicBlock::iterator I) {
  ...
  if (I->getOpcode() != JJ::BCOND && I->getOpcode() != JJ::BCONDA)
    return false;
  ...
  if (!isFloat(MI) && MI->getOpcode() != JJ::FBCOND)
    return false;
...
bool ErrataWorkaround::runOnMachineFunction(MachineFunction &MF) {
  bool Changed = false;
  ST = &MF.getSubtarget<Mwv208Subtarget>();
...
bool InsertNOPLoad::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    for (auto MBBI = MBB.begin(), E = MBB.end(); MBBI != E; ++MBBI) {
      ...
      if (Opcode >= JJ::LDDArr && Opcode <= JJ::LDrr) {
        MachineBasicBlock::iterator NMBBI = std::next(MBBI);
        BuildMI(MBB, NMBBI, DL, TII.get(JJ::NOP));
        Modified = true;
      }
    }
  }
...
bool DetectRoundChange::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : MBB) {
      unsigned Opcode = MI.getOpcode();
      if (Opcode == JJ::CALL && MI.getNumOperands() > 0) {
...
bool FixAllFDIVSQRT::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    for (auto MBBI = MBB.begin(), E = MBB.end(); MBBI != E; ++MBBI) {
      ...
      // pipeline.
      if (Opcode == JJ::FSQRTD || Opcode == JJ::FDIVD) {
        for (int InsertedCount = 0; InsertedCount < 5; InsertedCount++)
          BuildMI(MBB, MBBI, DL, TII.get(JJ::NOP));
