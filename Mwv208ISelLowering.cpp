...
static bool CC_Mwv208_Assign_SRet(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
...
static bool CC_Mwv208_Assign_Split_64(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                                      CCValAssign::LocInfo &LocInfo,
                                      ISD::ArgFlagsTy &ArgFlags,
                                      CCState &State) {
  static const MCPhysReg RegList[] = {JJ::I0, JJ::I1, JJ::I2,
                                      JJ::I3, JJ::I4, JJ::I5};
...
static bool CC_Mwv208_Assign_Ret_Split_64(unsigned &ValNo, MVT &ValVT,
                                          MVT &LocVT,
                                          CCValAssign::LocInfo &LocInfo,
                                          ISD::ArgFlagsTy &ArgFlags,
                                          CCState &State) {
...
static bool Analyze_CC_Mwv20864_Full(bool IsReturn, unsigned &ValNo, MVT &ValVT,
...
static bool Analyze_CC_Mwv20864_Half(bool IsReturn, unsigned &ValNo, MVT &ValVT,
                                     MVT &LocVT, CCValAssign::LocInfo &LocInfo,
                                     ISD::ArgFlagsTy &ArgFlags,
                                     CCState &State) {
  ...
  if (LocVT == MVT::f32 && Offset < 16 * 8) {
    ...
    State.addLoc(
        CCValAssign::getReg(ValNo, ValVT, JJ::F0 + Offset / 4, LocVT, LocInfo));
    return true;
  }

  if (LocVT == MVT::i32 && Offset < 6 * 8) {
    // Promote integers to %i0-%i5, using half the register.
    unsigned Reg = JJ::I0 + Offset / 8;
    LocVT = MVT::i64;
...
static bool CC_Mwv20864_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                             CCValAssign::LocInfo &LocInfo,
                             ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_Mwv20864_Full(false, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                  State);
}

static bool CC_Mwv20864_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                             CCValAssign::LocInfo &LocInfo,
                             ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_Mwv20864_Half(false, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                  State);
}

static bool RetCC_Mwv20864_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                                CCValAssign::LocInfo &LocInfo,
                                ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_Mwv20864_Full(true, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                  State);
}

static bool RetCC_Mwv20864_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                                CCValAssign::LocInfo &LocInfo,
                                ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_Mwv20864_Half(true, ValNo, ValVT, LocVT, LocInfo, ArgFlags,
                                  State);
}

#include "Mwv208GenCallingConv.inc"

// The calling conventions in Mwv208CallingConv.td are described in terms of the
// callee's register window. This function translates registers to the
// corresponding caller window %o register.
static unsigned toCallerWindow(unsigned Reg) {
  static_assert(JJ::I0 + 7 == JJ::I7 && JJ::O0 + 7 == JJ::O7,
                "Unexpected enum");
  if (Reg >= JJ::I0 && Reg <= JJ::I7)
    return Reg - JJ::I0 + JJ::O0;
  return Reg;
}

bool Mwv208TargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool isVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context,
    const Type *RetTy) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, isVarArg, MF, RVLocs, Context);
  return CCInfo.CheckReturn(Outs, Subtarget->is64Bit() ? RetCC_Mwv20864
                                                       : RetCC_Mwv20832);
}

SDValue
Mwv208TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                  bool IsVarArg,
...
SDValue Mwv208TargetLowering::LowerReturn_32(
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Mwv20832);
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    Mwv208MachineFunctionInfo *SFI = MF.getInfo<Mwv208MachineFunctionInfo>();
    Register Reg = SFI->getSRetReturnReg();
    if (!Reg)
      llvm_unreachable("sret virtual register not created in the entry block");
    auto PtrVT = getPointerTy(DAG.getDataLayout());
    SDValue Val = DAG.getCopyFromReg(Chain, DL, Reg, PtrVT);
    Chain = DAG.getCopyToReg(Chain, DL, JJ::I0, Val, Glue);
    Glue = Chain.getValue(1);
    RetOps.push_back(DAG.getRegister(JJ::I0, PtrVT));
...
SDValue Mwv208TargetLowering::LowerReturn_64(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs,
    const SmallVectorImpl<SDValue> &OutVals, const SDLoc &DL,
    SelectionDAG &DAG) const {
  ...
  // Analyze return values.
  CCInfo.AnalyzeReturn(Outs, RetCC_Mwv20864);
  ...
  for (unsigned i = 0; i != RVLocs.size(); ++i) {
    ...
    if (VA.getValVT() == MVT::i32 && VA.needsCustom()) {
      ...
      // Handle both at once.
      if (i + 1 < RVLocs.size() &&
          RVLocs[i + 1].getLocReg() == VA.getLocReg()) {
        SDValue NV =
            DAG.getNode(ISD::ZERO_EXTEND, DL, MVT::i64, OutVals[i + 1]);
...
SDValue Mwv208TargetLowering::LowerFormalArguments(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
...
SDValue Mwv208TargetLowering::LowerFormalArguments_32(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Mwv20832);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i, ++InIdx) {
    ...
    if (Ins[InIdx].Flags.isSRet()) {
      if (InIdx != 0)
        report_fatal_error("mwv208 only supports sret on the first parameter");
      // Get SRet from [%fp+64].
    ...
    if (VA.isRegLoc()) {
      if (VA.needsCustom()) {
        ...
        Register VRegHi = RegInfo.createVirtualRegister(&JJ::IntRegsRegClass);
        ...
        if (NextVA.isMemLoc()) {
          ...
          Register loReg =
              MF.addLiveIn(NextVA.getLocReg(), &JJ::IntRegsRegClass);
      ...
      Register VReg = RegInfo.createVirtualRegister(&JJ::IntRegsRegClass);
    ...
    if (VA.getValVT() == MVT::i32 || VA.getValVT() == MVT::f32) {
      ...
      report_fatal_error("MWV208v8 does not handle f128 in calls; "
                         "pass indirectly");
    } else {
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    ...
    Register Reg = SFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(&JJ::IntRegsRegClass);
  ...
  if (isVarArg) {
    static const MCPhysReg ArgRegs[] = {JJ::I0, JJ::I1, JJ::I2,
                                        JJ::I3, JJ::I4, JJ::I5};
...
SDValue Mwv208TargetLowering::LowerFormalArguments_64(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();

  // Analyze arguments according to CC_Mwv20864.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Mwv20864);
  ...
  for (const CCValAssign &VA : ArgLocs) {
    ...
    assert(VA.isMemLoc());
    // The CC_Mwv20864_Full/Half functions compute stack offsets relative to the
    // beginning of the arguments area at %fp+BIAS+128.
    unsigned Offset = VA.getLocMemOffset() + ArgArea;
    unsigned ValSize = VA.getValVT().getSizeInBits() / 8;
    // Adjust offset for extended arguments, MWV208 is big-endian.
  ...
  for (; ArgOffset < 6 * 8; ArgOffset += 8) {
    Register VReg = MF.addLiveIn(JJ::I0 + ArgOffset / 8, &JJ::I64RegsRegClass);
...
static bool isAnyArgRegReserved(const Mwv208RegisterInfo *TRI,
  ...
  bool Outgoing =
      llvm::any_of(JJ::GPROutgoingArgRegClass, [TRI, &MF](MCPhysReg r) {
        return TRI->isReservedReg(MF, r);
      });
  bool Incoming =
      llvm::any_of(JJ::GPRIncomingArgRegClass, [TRI, &MF](MCPhysReg r) {
        return TRI->isReservedReg(MF, r);
      });
...
static void emitReservedArgRegCallError(const MachineFunction &MF) {
  ...
  F.getContext().diagnose(DiagnosticInfoUnsupported{
      F, ("MWV208 doesn't support"
          " function calls if any of the argument registers is reserved.")});
}

SDValue
Mwv208TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
...
bool Mwv208TargetLowering::IsEligibleForTailCallOptimization(
...
SDValue
Mwv208TargetLowering::LowerCall_32(TargetLowering::CallLoweringInfo &CLI,
                                   SmallVectorImpl<SDValue> &InVals) const {
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_Mwv20832);
  ...
  for (unsigned i = 0, realArgIdx = 0, byvalArgIdx = 0, e = ArgLocs.size();
    ...
    if (Flags.isSRet()) {
      ...
      SDValue StackPtr = DAG.getRegister(JJ::O6, MVT::i32);
  ...
  unsigned TF = isPositionIndependent() ? Mwv208MCExpr::VK_Mwv208_WPLT30
                                        : Mwv208MCExpr::VK_Mwv208_WDISP30;
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee))
  ...
  const Mwv208RegisterInfo *TRI = Subtarget->getRegisterInfo();
  const uint32_t *Mask =
      ((hasReturnsTwice)
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_Mwv20832);
...
//
// The MWV208 v9 ABI requires that floating point arguments are treated the same
...
static void fixupVariableFloatArgs(SmallVectorImpl<CCValAssign> &ArgLocs,
  ...
  for (CCValAssign &VA : ArgLocs) {
    ...
    Register firstReg = (ValTy == MVT::f64) ? JJ::D0 : JJ::Q0;
...
SDValue
Mwv208TargetLowering::LowerCall_64(TargetLowering::CallLoweringInfo &CLI,
                                   SmallVectorImpl<SDValue> &InVals) const {
  SelectionDAG &DAG = CLI.DAG;
  SDLoc DL = CLI.DL;
  SDValue Chain = CLI.Chain;
  ...
  CCState CCInfo(CLI.CallConv, CLI.IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(CLI.Outs, CC_Mwv20864);

  CLI.IsTailCall = CLI.IsTailCall && IsEligibleForTailCallOptimization(
                                         CCInfo, CLI, DAG.getMachineFunction());

  // Get the size of the outgoing arguments stack space requirement.
  // The stack offset computed by CC_Mwv20864 includes all arguments.
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    case CCValAssign::BCvt:
      ...
      // MWV208 does not support i128 natively. Lower it into two i64, see
      // below.
  ...
  RVInfo.AnalyzeCallResult(CLI.Ins, RetCC_Mwv20864);
...
Mwv208TargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
...
/// intCondCCodeToRcond - Convert a DAG integer condition code to a MWV208
/// rcond condition.
static JJCC::CondCodes intCondCCodeToRcond(ISD::CondCode CC) {
  switch (CC) {
  default:
...
/// IntCondCCodeToICC - Convert a DAG integer condition code to a MWV208 ICC
/// condition.
static JJCC::CondCodes IntCondCCodeToICC(ISD::CondCode CC) {
  switch (CC) {
  default:
...
/// FPCondCCodeToFCC - Convert a DAG floatingp oint condition code to a MWV208
/// FCC condition.
static JJCC::CondCodes FPCondCCodeToFCC(ISD::CondCode CC) {
  switch (CC) {
  default:
...
Mwv208TargetLowering::Mwv208TargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i32, &JJ::IntRegsRegClass);
  if (!Subtarget->useSoftFloat()) {
    addRegisterClass(MVT::f32, &JJ::FPRegsRegClass);
    addRegisterClass(MVT::f64, &JJ::DFPRegsRegClass);
    addRegisterClass(MVT::f128, &JJ::QFPRegsRegClass);
  }
  if (Subtarget->is64Bit()) {
    addRegisterClass(MVT::i64, &JJ::I64RegsRegClass);
  } else {
    // On 32bit mwv208, we define a double-register 32bit register
    // class, as well. This is modeled in LLVM as a 2-vector of i32.
    addRegisterClass(MVT::v2i32, &JJ::IntPairRegClass);
  ...
  // Mwv208 doesn't have i1 sign extending load
  ...
  // Mwv208 doesn't have sext_inreg, replace them with shl/sra
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i16, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i8, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i1, Expand);

  // Mwv208 has no REM or DIVREM operations.
  ...
  // Mwv208 has no select or setcc: expand to SELECT_CC.
  ...
  // Mwv208 doesn't have BRCOND either, it has BR_CC.
  ...
  if (!Subtarget->isV9()) {
    // Mwv208V8 does not have FNEGD and FABSD.
  ...
  setStackPointerRegisterToSaveRestore(JJ::O6);
...
bool Mwv208TargetLowering::useSoftFloat() const {
...
EVT Mwv208TargetLowering::getSetCCResultType(const DataLayout &, LLVMContext &,
                                             EVT VT) const {
  if (!VT.isVector())
    return MVT::i32;
...
// set LHS/RHS and JJCC to the LHS/RHS of the setcc and JJCC to the condition.
static void LookThroughSetCC(SDValue &LHS, SDValue &RHS, ISD::CondCode CC,
                             unsigned &JJCC) {
  if (isNullConstant(RHS) && CC == ISD::SETNE &&
    ...
    JJCC = LHS.getConstantOperandVal(2);
...
SDValue Mwv208TargetLowering::withTargetFlags(SDValue Op, unsigned TF,
                                              SelectionDAG &DAG) const {
...
SDValue Mwv208TargetLowering::makeHiLoPair(SDValue Op, unsigned HiTF,
                                           unsigned LoTF,
                                           SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue Mwv208TargetLowering::makeAddress(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = getPointerTy(DAG.getDataLayout());

  // Handle PIC mode first. MWV208 needs a got load for every variable!
  if (isPositionIndependent()) {
    ...
    if (picLevel == PICLevel::SmallPIC) {
      ...
      Idx =
          DAG.getNode(SPISD::Lo, DL, Op.getValueType(),
                      withTargetFlags(Op, Mwv208MCExpr::VK_Mwv208_GOT13, DAG));
    } else {
      // This is the pic32 code model, the GOT is known to be smaller than 4GB.
      Idx = makeHiLoPair(Op, Mwv208MCExpr::VK_Mwv208_GOT22,
                         Mwv208MCExpr::VK_Mwv208_GOT10, DAG);
    }
  ...
  case CodeModel::Small:
    // abs32.
    return makeHiLoPair(Op, Mwv208MCExpr::VK_Mwv208_HI,
                        Mwv208MCExpr::VK_Mwv208_LO, DAG);
  case CodeModel::Medium: {
    // abs44.
    SDValue H44 = makeHiLoPair(Op, Mwv208MCExpr::VK_Mwv208_H44,
                               Mwv208MCExpr::VK_Mwv208_M44, DAG);
    H44 = DAG.getNode(ISD::SHL, DL, VT, H44, DAG.getConstant(12, DL, MVT::i32));
    SDValue L44 = withTargetFlags(Op, Mwv208MCExpr::VK_Mwv208_L44, DAG);
    L44 = DAG.getNode(SPISD::Lo, DL, VT, L44);
    return DAG.getNode(ISD::ADD, DL, VT, H44, L44);
  }
  case CodeModel::Large: {
    // abs64.
    SDValue Hi = makeHiLoPair(Op, Mwv208MCExpr::VK_Mwv208_HH,
                              Mwv208MCExpr::VK_Mwv208_HM, DAG);
    Hi = DAG.getNode(ISD::SHL, DL, VT, Hi, DAG.getConstant(32, DL, MVT::i32));
    SDValue Lo = makeHiLoPair(Op, Mwv208MCExpr::VK_Mwv208_HI,
                              Mwv208MCExpr::VK_Mwv208_LO, DAG);
    return DAG.getNode(ISD::ADD, DL, VT, Hi, Lo);
  }
  }
...
SDValue Mwv208TargetLowering::LowerGlobalAddress(SDValue Op,
                                                 SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue Mwv208TargetLowering::LowerConstantPool(SDValue Op,
                                                SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue Mwv208TargetLowering::LowerBlockAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue Mwv208TargetLowering::LowerGlobalTLSAddress(SDValue Op,
                                                    SelectionDAG &DAG) const {
  ...
  if (model == TLSModel::GeneralDynamic || model == TLSModel::LocalDynamic) {
    unsigned HiTF = ((model == TLSModel::GeneralDynamic)
                         ? Mwv208MCExpr::VK_Mwv208_TLS_GD_HI22
                         : Mwv208MCExpr::VK_Mwv208_TLS_LDM_HI22);
    unsigned LoTF = ((model == TLSModel::GeneralDynamic)
                         ? Mwv208MCExpr::VK_Mwv208_TLS_GD_LO10
                         : Mwv208MCExpr::VK_Mwv208_TLS_LDM_LO10);
    unsigned addTF = ((model == TLSModel::GeneralDynamic)
                          ? Mwv208MCExpr::VK_Mwv208_TLS_GD_ADD
                          : Mwv208MCExpr::VK_Mwv208_TLS_LDM_ADD);
    unsigned callTF = ((model == TLSModel::GeneralDynamic)
                           ? Mwv208MCExpr::VK_Mwv208_TLS_GD_CALL
                           : Mwv208MCExpr::VK_Mwv208_TLS_LDM_CALL);
    ...
    Chain = DAG.getCopyToReg(Chain, DL, JJ::O0, Argument, InGlue);
    InGlue = Chain.getValue(1);
    ...
    InGlue = Chain.getValue(1);
    SDValue Ret = DAG.getCopyFromReg(Chain, DL, JJ::O0, PtrVT, InGlue);
    ...
    SDValue Hi = DAG.getNode(
        SPISD::Hi, DL, PtrVT,
        withTargetFlags(Op, Mwv208MCExpr::VK_Mwv208_TLS_LDO_HIX22, DAG));
    SDValue Lo = DAG.getNode(
        SPISD::Lo, DL, PtrVT,
        withTargetFlags(Op, Mwv208MCExpr::VK_Mwv208_TLS_LDO_LOX10, DAG));
    HiLo = DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);
    return DAG.getNode(
        SPISD::TLS_ADD, DL, PtrVT, Ret, HiLo,
        withTargetFlags(Op, Mwv208MCExpr::VK_Mwv208_TLS_LDO_ADD, DAG));
  }

  if (model == TLSModel::InitialExec) {
    unsigned ldTF = ((PtrVT == MVT::i64) ? Mwv208MCExpr::VK_Mwv208_TLS_IE_LDX
                                         : Mwv208MCExpr::VK_Mwv208_TLS_IE_LD);
    ...
    SDValue TGA = makeHiLoPair(Op, Mwv208MCExpr::VK_Mwv208_TLS_IE_HI22,
                               Mwv208MCExpr::VK_Mwv208_TLS_IE_LO10, DAG);
    SDValue Ptr = DAG.getNode(ISD::ADD, DL, PtrVT, Base, TGA);
    SDValue Offset = DAG.getNode(SPISD::TLS_LD, DL, PtrVT, Ptr,
                                 withTargetFlags(Op, ldTF, DAG));
    return DAG.getNode(
        SPISD::TLS_ADD, DL, PtrVT, DAG.getRegister(JJ::G7, PtrVT), Offset,
        withTargetFlags(Op, Mwv208MCExpr::VK_Mwv208_TLS_IE_ADD, DAG));
  }

  assert(model == TLSModel::LocalExec);
  SDValue Hi = DAG.getNode(
      SPISD::Hi, DL, PtrVT,
      withTargetFlags(Op, Mwv208MCExpr::VK_Mwv208_TLS_LE_HIX22, DAG));
  SDValue Lo = DAG.getNode(
      SPISD::Lo, DL, PtrVT,
      withTargetFlags(Op, Mwv208MCExpr::VK_Mwv208_TLS_LE_LOX10, DAG));
  SDValue Offset = DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);

  return DAG.getNode(ISD::ADD, DL, PtrVT, DAG.getRegister(JJ::G7, PtrVT),
                     Offset);
}

SDValue Mwv208TargetLowering::LowerF128_LibCallArg(SDValue Chain,
...
SDValue Mwv208TargetLowering::LowerF128Op(SDValue Op, SelectionDAG &DAG,
  ...
  CLI.setDebugLoc(SDLoc(Op)).setChain(Chain).setCallee(CallingConv::C, RetTyABI,
...
SDValue Mwv208TargetLowering::LowerF128Compare(SDValue LHS, SDValue RHS,
                                               unsigned &JJCC, const SDLoc &DL,
                                               SelectionDAG &DAG) const {
  ...
  case JJCC::FCC_E:
    LibCall = is64Bit ? "_Qp_feq" : "_Q_feq";
    break;
  case JJCC::FCC_NE:
    LibCall = is64Bit ? "_Qp_fne" : "_Q_fne";
    break;
  case JJCC::FCC_L:
    LibCall = is64Bit ? "_Qp_flt" : "_Q_flt";
    break;
  case JJCC::FCC_G:
    LibCall = is64Bit ? "_Qp_fgt" : "_Q_fgt";
    break;
  case JJCC::FCC_LE:
    LibCall = is64Bit ? "_Qp_fle" : "_Q_fle";
    break;
  case JJCC::FCC_GE:
    LibCall = is64Bit ? "_Qp_fge" : "_Q_fge";
    break;
  case JJCC::FCC_UL:
  case JJCC::FCC_ULE:
  case JJCC::FCC_UG:
  ...
  case JJCC::FCC_LG:
  case JJCC::FCC_UE:
    LibCall = is64Bit ? "_Qp_cmp" : "_Q_cmp";
    break;
  }
  ...
  TargetLowering::CallLoweringInfo CLI(DAG);
  CLI.setDebugLoc(DL).setChain(Chain).setCallee(CallingConv::C, RetTy, Callee,
                                                std::move(Args));
...
static SDValue LowerF128_FPEXTEND(SDValue Op, SelectionDAG &DAG,
                                  const Mwv208TargetLowering &TLI) {

  if (Op.getOperand(0).getValueType() == MVT::f64)
    return TLI.LowerF128Op(Op, DAG, TLI.getLibcallName(RTLIB::FPEXT_F64_F128),
                           1);

  if (Op.getOperand(0).getValueType() == MVT::f32)
    return TLI.LowerF128Op(Op, DAG, TLI.getLibcallName(RTLIB::FPEXT_F32_F128),
                           1);
...
static SDValue LowerF128_FPROUND(SDValue Op, SelectionDAG &DAG,
                                 const Mwv208TargetLowering &TLI) {
  ...
  if (Op.getValueType() == MVT::f64)
    return TLI.LowerF128Op(Op, DAG, TLI.getLibcallName(RTLIB::FPROUND_F128_F64),
                           1);
  if (Op.getValueType() == MVT::f32)
    return TLI.LowerF128Op(Op, DAG, TLI.getLibcallName(RTLIB::FPROUND_F128_F32),
                           1);
...
static SDValue LowerFP_TO_SINT(SDValue Op, SelectionDAG &DAG,
  ...
  if (Op.getOperand(0).getValueType() == MVT::f128 &&
    ...
    const char *libName = TLI.getLibcallName(
...
static SDValue LowerBR_CC(SDValue Op, SelectionDAG &DAG,
                          const Mwv208TargetLowering &TLI, bool hasHardQuad,
  ...
  SDLoc dl(Op);
  unsigned Opc, JJCC = ~0U;

  // If this is a br_cc of a "setcc", and if the setcc got lowered into
  // an CMP[IF]CC/SELECT_[IF]CC pair, find the original compared values.
  LookThroughSetCC(LHS, RHS, CC, JJCC);
  ...
  if (LHS.getValueType().isInteger()) {
    ...
    if (!hasHardQuad && LHS.getValueType() == MVT::f128) {
      if (JJCC == ~0U)
        JJCC = FPCondCCodeToFCC(CC);
      CompareFlag = TLI.LowerF128Compare(LHS, RHS, JJCC, dl, DAG);
  ...
  return DAG.getNode(Opc, dl, MVT::Other, Chain, Dest,
                     DAG.getConstant(JJCC, dl, MVT::i32), CompareFlag);
}
...
static SDValue LowerSELECT_CC(SDValue Op, SelectionDAG &DAG,
  ...
  if (LHS.getValueType().isInteger()) {
    ...
    CompareFlag = DAG.getNode(SPISD::CMPICC, dl, MVT::Glue, LHS, RHS);
    Opc =
        LHS.getValueType() == MVT::i32 ? SPISD::SELECT_ICC : SPISD::SELECT_XCC;
    if (JJCC == ~0U)
      JJCC = IntCondCCodeToICC(CC);
  } else {
    if (!hasHardQuad && LHS.getValueType() == MVT::f128) {
...
static SDValue LowerVASTART(SDValue Op, SelectionDAG &DAG,
  ...
  SDLoc DL(Op);
  SDValue Offset =
      DAG.getNode(ISD::ADD, DL, PtrVT, DAG.getRegister(JJ::I6, PtrVT),
...
static SDValue LowerDYNAMIC_STACKALLOC(SDValue Op, SelectionDAG &DAG,
  ...
  unsigned SPReg = JJ::O6;
  SDValue JJ = DAG.getCopyFromReg(Chain, dl, SPReg, VT);
  ...
  if (Subtarget->is64Bit()) {
    ...
    // On Mwv20832, the size of the spill area is 92. Unfortunately,
    ...
    Size = DAG.getNode(ISD::ADD, dl, VT, Size, DAG.getConstant(8, dl, VT));
    regSpillArea = 96;
  }
  ...
  // we rebias the JJ later on.
  SDValue StartOfUsableStack = DAG.getNode(
      ISD::ADD, dl, VT, JJ, DAG.getConstant(regSpillArea + Bias, dl, VT));
  ...
  Chain = DAG.getCopyToReg(JJ.getValue(1), dl, SPReg, NewSP);
...
static SDValue getFLUSHW(SDValue Op, SelectionDAG &DAG) {
  SDLoc dl(Op);
  SDValue Chain =
      DAG.getNode(SPISD::FLUSHW, dl, MVT::Other, DAG.getEntryNode());
  return Chain;
}
...
static SDValue LowerRETURNADDR(SDValue Op, SelectionDAG &DAG,
  ...
  if (depth == 0) {
    auto PtrVT = TLI.getPointerTy(DAG.getDataLayout());
    Register RetReg = MF.addLiveIn(JJ::I7, TLI.getRegClassFor(PtrVT));
...
static SDValue LowerF64Op(SDValue SrcReg64, const SDLoc &dl, SelectionDAG &DAG,
                          unsigned opcode) {
  assert(SrcReg64.getValueType() == MVT::f64 &&
  ...
  SDValue Hi32 =
      DAG.getTargetExtractSubreg(JJ::sub_even, dl, MVT::f32, SrcReg64);
  SDValue Lo32 =
      DAG.getTargetExtractSubreg(JJ::sub_odd, dl, MVT::f32, SrcReg64);
  ...
  SDValue DstReg64 =
      SDValue(DAG.getMachineNode(TargetOpcode::IMPLICIT_DEF, dl, MVT::f64), 0);
  DstReg64 =
      DAG.getTargetInsertSubreg(JJ::sub_even, dl, MVT::f64, DstReg64, Hi32);
  DstReg64 =
      DAG.getTargetInsertSubreg(JJ::sub_odd, dl, MVT::f64, DstReg64, Lo32);
  return DstReg64;
}
...
static SDValue LowerF128Load(SDValue Op, SelectionDAG &DAG) {
  ...
  SDValue LoPtr = DAG.getNode(ISD::ADD, dl, addrVT, LdNode->getBasePtr(),
                              DAG.getConstant(8, dl, addrVT));
  SDValue Lo64 =
      DAG.getLoad(MVT::f64, dl, LdNode->getChain(), LoPtr,
                  LdNode->getPointerInfo().getWithOffset(8), Alignment);

  SDValue SubRegEven = DAG.getTargetConstant(JJ::sub_even64, dl, MVT::i32);
  SDValue SubRegOdd = DAG.getTargetConstant(JJ::sub_odd64, dl, MVT::i32);
...
static SDValue LowerF128Store(SDValue Op, SelectionDAG &DAG) {
  ...
  SDNode *Hi64 = DAG.getMachineNode(TargetOpcode::EXTRACT_SUBREG, dl, MVT::f64,
                                    StNode->getValue(), SubRegEven);
  SDNode *Lo64 = DAG.getMachineNode(TargetOpcode::EXTRACT_SUBREG, dl, MVT::f64,
  ...
  OutChains[0] =
      DAG.getStore(StNode->getChain(), dl, SDValue(Hi64, 0),
                   StNode->getBasePtr(), StNode->getPointerInfo(), Alignment);
  EVT addrVT = StNode->getBasePtr().getValueType();
  SDValue LoPtr = DAG.getNode(ISD::ADD, dl, addrVT, StNode->getBasePtr(),
                              DAG.getConstant(8, dl, addrVT));
  OutChains[1] =
      DAG.getStore(StNode->getChain(), dl, SDValue(Lo64, 0), LoPtr,
                   StNode->getPointerInfo().getWithOffset(8), Alignment);
...
static SDValue LowerSTORE(SDValue Op, SelectionDAG &DAG) {
  ...
  if (MemVT == MVT::i64) {
    ...
    SDValue Chain =
        DAG.getStore(St->getChain(), dl, Val, St->getBasePtr(),
...
static SDValue LowerFNEGorFABS(SDValue Op, SelectionDAG &DAG, bool isV9) {
  ...
  SDValue Hi64 =
      DAG.getTargetExtractSubreg(JJ::sub_even64, dl, MVT::f64, SrcReg128);
  SDValue Lo64 =
      DAG.getTargetExtractSubreg(JJ::sub_odd64, dl, MVT::f64, SrcReg128);
  ...
  SDValue DstReg128 =
      SDValue(DAG.getMachineNode(TargetOpcode::IMPLICIT_DEF, dl, MVT::f128), 0);
  DstReg128 =
      DAG.getTargetInsertSubreg(JJ::sub_even64, dl, MVT::f128, DstReg128, Hi64);
  DstReg128 =
      DAG.getTargetInsertSubreg(JJ::sub_odd64, dl, MVT::f128, DstReg128, Lo64);
  return DstReg128;
}
...
SDValue Mwv208TargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                      SelectionDAG &DAG) const {
  ...
  case Intrinsic::thread_pointer: {
    EVT PtrVT = getPointerTy(DAG.getDataLayout());
    return DAG.getRegister(JJ::G7, PtrVT);
  }
  }
}

SDValue Mwv208TargetLowering::LowerOperation(SDValue Op,
                                             SelectionDAG &DAG) const {
...
SDValue Mwv208TargetLowering::bitcastConstantFPToInt(ConstantFPSDNode *C,
                                                     const SDLoc &DL,
                                                     SelectionDAG &DAG) const {
...
SDValue
Mwv208TargetLowering::PerformBITCASTCombine(SDNode *N,
...
SDValue Mwv208TargetLowering::PerformDAGCombine(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
  switch (N->getOpcode()) {
  default:
    break;
...
MachineBasicBlock *
Mwv208TargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                                  MachineBasicBlock *BB) const {
  switch (MI.getOpcode()) {
  default:
    llvm_unreachable("Unknown SELECT_CC!");
  case JJ::SELECT_CC_Int_ICC:
  case JJ::SELECT_CC_FP_ICC:
  case JJ::SELECT_CC_DFP_ICC:
  case JJ::SELECT_CC_QFP_ICC:
    if (Subtarget->isV9())
      return expandSelectCC(MI, BB, JJ::BPICC);
    return expandSelectCC(MI, BB, JJ::BCOND);
  case JJ::SELECT_CC_Int_XCC:
  case JJ::SELECT_CC_FP_XCC:
  case JJ::SELECT_CC_DFP_XCC:
  case JJ::SELECT_CC_QFP_XCC:
    return expandSelectCC(MI, BB, JJ::BPXCC);
  case JJ::SELECT_CC_Int_FCC:
  case JJ::SELECT_CC_FP_FCC:
  case JJ::SELECT_CC_DFP_FCC:
  case JJ::SELECT_CC_QFP_FCC:
    if (Subtarget->isV9())
      return expandSelectCC(MI, BB, JJ::FBCOND_V9);
    return expandSelectCC(MI, BB, JJ::FBCOND);
  }
}

MachineBasicBlock *
Mwv208TargetLowering::expandSelectCC(MachineInstr &MI, MachineBasicBlock *BB,
                                     unsigned BROpcode) const {
  const TargetInstrInfo &TII = *Subtarget->getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();
  unsigned CC = (JJCC::CondCodes)MI.getOperand(3).getImm();
  ...
  BuildMI(*SinkMBB, SinkMBB->begin(), dl, TII.get(JJ::PHI),
...
//===----------------------------------------------------------------------===//
//                         Mwv208 Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
Mwv208TargetLowering::ConstraintType
Mwv208TargetLowering::getConstraintType(StringRef Constraint) const {
...
Mwv208TargetLowering::getSingleConstraintMatchWeight(
...
void Mwv208TargetLowering::LowerAsmOperandForConstraint(
...
Mwv208TargetLowering::getRegForInlineAsmConstraint(
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r':
      if (VT == MVT::v2i32)
        return std::make_pair(0U, &JJ::IntPairRegClass);
      else if (Subtarget->is64Bit())
        return std::make_pair(0U, &JJ::I64RegsRegClass);
      else
        return std::make_pair(0U, &JJ::IntRegsRegClass);
    case 'f':
      if (VT == MVT::f32 || VT == MVT::i32)
        return std::make_pair(0U, &JJ::FPRegsRegClass);
      else if (VT == MVT::f64 || VT == MVT::i64)
        return std::make_pair(0U, &JJ::LowDFPRegsRegClass);
      else if (VT == MVT::f128)
        return std::make_pair(0U, &JJ::LowQFPRegsRegClass);
    ...
    case 'e':
      if (VT == MVT::f32 || VT == MVT::i32)
        ...
        return std::make_pair(0U, &JJ::DFPRegsRegClass);
      else if (VT == MVT::f128)
        return std::make_pair(0U, &JJ::QFPRegsRegClass);
      // This will generate an error message
      return std::make_pair(0U, nullptr);
    }
  }
  ...
  if (Subtarget->is64Bit() && VT == MVT::i64) {
    assert(ResultPair.second == &JJ::IntRegsRegClass &&
           "Unexpected register class");
    return std::make_pair(ResultPair.first, &JJ::I64RegsRegClass);
  }

  return ResultPair;
}

bool Mwv208TargetLowering::isOffsetFoldingLegal(
    const GlobalAddressSDNode *GA) const {
  // The Mwv208 target isn't yet aware of offsets.
  return false;
}

void Mwv208TargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  case ISD::READCYCLECOUNTER: {
    ...
    SDValue Lo = DAG.getCopyFromReg(N->getOperand(0), dl, JJ::ASR23, MVT::i32);
    SDValue Hi = DAG.getCopyFromReg(Lo, dl, JJ::G0, MVT::i32);
    SDValue Ops[] = {Lo, Hi};
...
bool Mwv208TargetLowering::useLoadStackGuardNode(const Module &M) const {
...
void Mwv208TargetLowering::insertSSPDeclarations(Module &M) const {
  if (!Subtarget->isTargetLinux())
    return TargetLowering::insertSSPDeclarations(M);
}

void Mwv208TargetLowering::AdjustInstrPostInstrSelection(MachineInstr &MI,
                                                         SDNode *Node) const {
  assert(MI.getOpcode() == JJ::SUBCCrr || MI.getOpcode() == JJ::SUBCCri);
  // If the result is dead, replace it with %g0.
  if (!Node->hasAnyUseOfValue(0))
    MI.getOperand(0).setReg(JJ::G0);
}
