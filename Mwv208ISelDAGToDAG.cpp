...
bool Mwv208DAGToDAGISel::SelectADDRrr(SDValue Addr, SDValue &R1, SDValue &R2) {
  ...
  R1 = Addr;
  R2 = CurDAG->getRegister(JJ::G0, TLI->getPointerTy(CurDAG->getDataLayout()));
  return true;
}
...
bool Mwv208DAGToDAGISel::tryInlineAsm(SDNode *N) {
  ...
  for (unsigned i = 0, e = N->getGluedNode() ? NumOps - 1 : NumOps; i < e;
    ...
    if ((!IsTiedToChangedOp && (!HasRC || RC != JJ::IntRegsRegClassID)) ||
        NumRegs != 2)
      continue;
    ...
    if (Flag.isRegDefKind() || Flag.isRegDefEarlyClobberKind()) {
      ...
      Register GPVR = MRI.createVirtualRegister(&JJ::IntPairRegClass);
      ...
      SDValue Sub0 =
          CurDAG->getTargetExtractSubreg(JJ::sub_even, dl, MVT::i32, RegCopy);
      SDValue Sub1 =
          CurDAG->getTargetExtractSubreg(JJ::sub_odd, dl, MVT::i32, RegCopy);
      SDValue T0 =
      ...
      SDValue Pair =
          SDValue(CurDAG->getMachineNode(
                      ...
                      {
                          CurDAG->getTargetConstant(JJ::IntPairRegClassID, dl,
                                                    MVT::i32),
                          T0,
                          CurDAG->getTargetConstant(JJ::sub_even, dl, MVT::i32),
                          T1,
                          CurDAG->getTargetConstant(JJ::sub_odd, dl, MVT::i32),
                      }),
                  0);
    ...
    if (PairedReg.getNode()) {
      ...
      if (IsTiedToChangedOp)
        ...
        Flag.setRegClass(JJ::IntPairRegClassID);
...
void Mwv208DAGToDAGISel::Select(SDNode *N) {
  ...
  case ISD::UDIV: {
    ...
    if (N->getOpcode() == ISD::SDIV) {
      TopPart = SDValue(
          CurDAG->getMachineNode(JJ::SRAri, dl, MVT::i32, DivLHS,
                                 CurDAG->getTargetConstant(31, dl, MVT::i32)),
          0);
    } else {
      TopPart = CurDAG->getRegister(JJ::G0, MVT::i32);
    }
    TopPart = CurDAG
                  ->getCopyToReg(CurDAG->getEntryNode(), dl, JJ::Y, TopPart,
                                 SDValue())
                  .getValue(1);

    // FIXME: Handle div by immediate.
    unsigned Opcode = N->getOpcode() == ISD::SDIV ? JJ::SDIVrr : JJ::UDIVrr;
...
/// SelectInlineAsmMemoryOperand - Implement addressing mode selection for
/// inline asm expressions.
bool Mwv208DAGToDAGISel::SelectInlineAsmMemoryOperand(
